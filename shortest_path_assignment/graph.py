...
class Graph:
    ...
    def dijkstra(self, s):
        ...
        while open.length() != 0:
            ...
            closed[v.value()] = (c, p)
            edges = self.get_edges(v.value())
            for e in edges:
                w = e.opposite(v.value())
                if w not in closed.keys():
                    newcost = e.element() + v.key()
                    if w not in location.keys():
                        pred[w] = v
...
# ver = g.vertices()
# dire = g.shortest_path(ver[13], ver[4])
